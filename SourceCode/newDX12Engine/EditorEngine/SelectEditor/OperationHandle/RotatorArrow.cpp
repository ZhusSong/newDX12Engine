...
#include "../../../EngineCode/Component/Mesh/PlaneMeshComponent.h"
...
void GRotatorArrow::CreateMesh()
{
	string MeshPathX = FEnginePathHelper::RelativeToAbsolutePath(
		FEnginePathHelper::GetEngineAssetPath() + "\\Handle\\RotateHandleX.fbx");

	string MeshPathY = FEnginePathHelper::RelativeToAbsolutePath(
		FEnginePathHelper::GetEngineAssetPath() + "\\Handle\\RotateHandleY.fbx");
...
void GRotatorArrow::SetAGValue(CMeshComponent* InComponent, float InValue)
{
	if (CMaterial* InMaterial = (*InComponent->GetMaterials())[0])
	{
...
void GRotatorArrow::SetAGValue(float InValue)
{
	SetAGValue(YPlaneComponent, InValue);
	SetAGValue(ZPlaneComponent, InValue);
	SetAGValue(XPlaneComponent, InValue);
}

void GRotatorArrow::ResetAGValue()
{
	SetAGValue(0.f);
}
...
void GRotatorArrow::ResetVisible(CCustomMeshComponent* InAxisComponent, bool bVisible)
{
	...
	GOperationHandleBase::ESelectAxisType AxisHandle = GetSelectAxis();
	switch (AxisHandle)
	{
	case GOperationHandleBase::SELECTAXIS_X:
			...
			if (bOperationHandleSelect)
			{
				...
				if (T2 != -1)
				{
					...
					//材质面片旋转
					{
						...
						SetAGValue(DelteRatio * SymbolMaterial);
					}
